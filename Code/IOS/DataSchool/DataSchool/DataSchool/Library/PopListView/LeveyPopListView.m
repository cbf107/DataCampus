...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(POPLISTVIEW_SCREENINSET, 
                                                                   ...
                                                                   rect.size.height - 2 * POPLISTVIEW_SCREENINSET - POPLISTVIEW_HEADER_HEIGHT - RADIUS-300)];
        _tableView.separatorColor = [UIColor colorWithWhite:0 alpha:.2];
        //_tableView.backgroundColor = [UIColor clearColor];
        _tableView.backgroundColor = [UIColor colorWithHexString:@"6d9ccb"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.imageView.image = [[_options objectAtIndex:row] objectForKey:@"name"];
    //cell.textLabel.text = [[_options objectAtIndex:row] objectForKey:@"money"];
    cell.textLabel.text = [[_options objectAtIndex:row] objectForKey:@"name"];
    ((LeveyPopListViewCell*)cell).mTextMoney.text = [[_options objectAtIndex:row] objectForKey:@"money"];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGRect bgRect = CGRectMake(POPLISTVIEW_SCREENINSET,
    ...
    [_title drawInRect:titleRect withFont:[UIFont systemFontOfSize:16.] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
